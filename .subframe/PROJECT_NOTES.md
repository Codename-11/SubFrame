...
_Generated by SubFrame onboarding on 2026-03-15_

**Vision:** ClawPort is a visual command centre that gives operators a unified dashboard to manage, monitor, and directly communicate with their OpenClaw AI agent team.
...
- OpenAI SDK (routed to Claude via OpenClaw gateway)
- Radix UI primitives
- Lucide React icons
...
**Key Decisions:**
- **CLI-based vision pipeline instead of WebSocket** (2025-01-01): The gateway WebSocket requires device keypair signing for operator.write scope. Since custom clients don't have device keys, vision routes through the CLI (openclaw gateway call chat.send + poll chat.history) rather than direct WebSocket connections.
- **No external charting or media libraries** (2025-01-01): All visualizations use native Web APIs (Canvas, SVG, MediaRecorder, AudioContext) to keep the dependency footprint small and maintain full control over rendering.
- **CSS custom properties for theming over Tailwind color classes** (2025-01-01): Five themes (Dark, Glass, Color, Light, System) are implemented via CSS custom properties in globals.css, allowing instant theme switching without class-based Tailwind color utilities.
- **localStorage for conversation persistence** (2025-01-01): Messages are stored as JSON in localStorage with base64 data URLs for media (not blob URLs) to survive page reloads without requiring server-side storage.
- **Auto-discovery of agents from workspace filesystem** (2025-01-01): Rather than requiring manual configuration, ClawPort scans the OpenClaw workspace for SOUL.md files to auto-discover agents, making any workspace work out of the box.
- **requireEnv() inside functions, not at module top level** (2025-01-01): Prevents imports from crashing during next build or test runs when environment variables are not set. All env access goes through lib/env.ts.

### [2026-03-24] SubFrame Server LAN Access Mode
SubFrame Server now supports two access modes instead of SSH-tunnel-only setup. SSH tunnel remains the recommended default because it keeps the server bound to `127.0.0.1`, while LAN mode is an explicit opt-in that rebinds the server to `0.0.0.0` for trusted-network use.

This was added mainly to make tablet and phone access practical, especially on Android where running an SSH tunnel is not a realistic default workflow. The UI now surfaces a clear warning for LAN mode, shows the local IP/QR flow, and keeps the security framing explicit: use LAN only on trusted home or office networks and disable it when not needed.

### [2026-03-24] Workspace Pills and Combined Terminal View
The workspace pills in the top bar now separate two signals instead of collapsing everything into "active AI": a green dot means one or more AI sessions are active in that workspace, while a blue badge shows how many terminals exist there. This makes it easier to see which workspaces are still "live" even when no agent is currently running.

The previous cross-project visibility model only supported manual per-terminal pinning. A temporary workspace-level "Mix" mode was added to the terminal tab bar so terminals from multiple projects in the current workspace can be viewed together without replacing the existing pin feature.

### [2026-03-25] Workspace Pill Identity Settings
Workspace pills now keep the current numeric `#1` style by default, but the top bar identity is configurable from Settings instead of being hard-coded. The global style can switch between index, short label, icon, or icon-plus-short-label, and each workspace stores its own optional short label and icon metadata.

Short labels are intentionally capped at 4 characters to keep the top bar compact. When a workspace has no custom short label, the UI falls back to an auto-generated monogram from the workspace name so label-based modes never collapse into blank pills.

The appearance controls are now split so theme-saving actions and workspace-pill controls are visually separate. Workspace order can also be changed directly from the top bar by holding a pill briefly and dragging it, which keeps the visible pill order as the canonical workspace order instead of hiding reordering inside the sidebar menu.

The pill display setting also moved from a single dropdown mode to a composable content matrix. Number, short label, and icon can now be enabled independently, which allows combinations like `#2 + LAB + rocket` without adding a separate preset for every combination.

For usability, `Index` now auto-pairs with the explicit custom short label when one exists. That means a workspace with short label `LAB` can render as `#2 LAB` even if the global content matrix only has `Index` enabled.

Terminal freeze controls were also filled in so grid mode is no longer missing parity with the tab strip. Grid headers now expose freeze/resume beside pop-out, the in-terminal paused banner became its own terminal setting (default on), and the SubFrame CLI install/remove actions are available from both the General CLI section and the Integrations CLI card.

The SubFrame Server settings card now exposes the full connection URL directly, rather than forcing users into the setup guide just to copy the tokenized link. This keeps the main server status area actionable for repeat mobile/LAN access.

That server card was then tightened further by separating the raw base URL from the tokenized connection URL and by surfacing the live workspace/project context right beside the client status. The intent is to make repeat remote usage more legible: one row for where the server is, one row for the shareable authenticated link, and one short status line for what the remote device is actually mirroring.

Remote web clients were also adjusted to hydrate from the live desktop session instead of trusting browser-local session state first. The desktop renderer now syncs the current project/session snapshot into the main process, `GET_TERMINAL_STATE` carries `projectPath`, and `web-entry.tsx` seeds the browser stores from that live snapshot before React renders so a phone/tablet opens much closer to the current desktop context.

That mobile/web hydration pass exposed a browser-compatibility issue: some Android browsers/WebViews still do not provide `crypto.randomUUID()`. The WebSocket transport now uses `randomUUID` when present, falls back to `crypto.getRandomValues()`, and only then drops to a timestamp/random string, so invoke calls like `WEB_SESSION_STATE` still work on older mobile clients.

The first hydration attempt also exposed a plumbing mistake: the main process was still registering handlers against raw `ipcMain`, while the web transport calls `routeInvoke()` against the router map. `index.ts` now builds a routable IPC wrapper up front and passes that into module `setupIPC()` calls, so browser-mode invoke handlers are actually reachable over WebSocket instead of existing only for the desktop renderer.

The next pass focused on making the base server URL practical on mobile instead of assuming the long tokenized URL is the only entry path. `web-entry.tsx` now treats `?token=` as an optional fast path rather than a requirement: opening the base URL shows a dedicated access screen where the browser can pair with a short code from Settings or accept a manually pasted token. After a successful direct-link connection, the token is moved into `sessionStorage` and removed from the address bar so reloads stay smooth without leaving the full auth token visible in the URL.

Port selection was also tightened so integrations can deliberately target a stable address when needed. The server now distinguishes between fixed and auto port modes: explicit `server.port` values are treated as a hard preference and surface bind errors back into Settings, while auto mode reuses the last successful port when it is still free and only falls back to a random open port if that remembered port is taken. This keeps repeat bookmarks and mobile home-screen shortcuts more stable without forcing the user into a permanent fixed-port configuration.

That web-client follow-up exposed another router gap: some managers were still self-registering their handlers straight onto raw `ipcMain` during `init()`, which meant desktop invokes worked but browser-mode `routeInvoke()` could not see channels like `get-ai-tool-config`. `settingsManager` and `aiToolManager` now register through the routed wrapper in `index.ts`, so the web client can hydrate the same settings and AI-tool state as the desktop renderer instead of tripping over missing handler errors.

The remote entry screen also needed desktop context before authentication. A small public bootstrap payload is now served over HTTP so the browser can pick up the active theme and current mirror target before the WebSocket session is established. That lets the pairing, connecting, reconnect, and takeover screens render with the same SubFrame palette and show which workspace/project they are about to mirror, instead of using a generic hard-coded shell.

### [2026-03-25] Remote Web Session Parity and Mobile Panel Access
The browser client is intended to drive the same live host instance, so the remote path needed a few parity fixes beyond simply serving the terminal output. The first gap was browser-originated `send` flows: some main-process handlers respond with `event.reply(...)`, and the WebSocket router originally passed a null event, which meant actions like `New Terminal` could execute on the host but never deliver `terminal-created` back to the remote renderer. The routed send path now maps `reply()` back through the event bridge so browser sessions can participate in the same follow-up event flow as Electron renderers.

The second gap was hydration scope. Remote clients were already getting terminal/session state, but not the host renderer’s broader UI context, so they could land in the right workspace/project while still feeling disconnected from the actual desktop view. The desktop renderer now syncs active right-panel/full-view state alongside the terminal session snapshot, and the web entry applies that during hydration so a phone/tablet can open closer to the current host view instead of rebuilding only from local browser state.

Phone web also needed a real surface for right-panel functionality. The mobile shell previously exposed only terminal/tasks/activity/settings, so GitHub, project, agent, and automation panels effectively disappeared on narrow web clients even though the data path existed. A dedicated `Panels` tab now hosts those panel families, and shared `activePanel` intents automatically route the phone shell into `Tasks` or `Panels` so panel actions from elsewhere in the app remain visible in remote/mobile mode.

Remote cursor tracking was added as a separate opt-in setting under SubFrame Server. When enabled, web pointer/touch events are forwarded to the host desktop and rendered as a lightweight overlay, which helps with mobile control, demos, and debugging remote interactions without making cursor mirroring always-on.

The next parity pass removed another source of drift: the phone `Terminal` tab no longer uses a bespoke single-terminal wrapper and instead renders the same `TerminalArea` used on desktop. That keeps terminal tabs, workspace/view tabs, grid mode, and full-view behavior on the same code path instead of maintaining a second mobile-only terminal UX.

Live session parity was also extended past the initial hydration moment. The browser now listens for ongoing `WEB_SESSION_SYNC` updates so terminal names/layout can settle to the host renderer's latest session state even if the desktop finishes restoring a moment after the remote client connects. This is especially important for keeping remote terminal labels consistent with the host instead of falling back to generic names.

Because remote access is becoming a normal workflow, the bottom activity bar now exposes a lightweight `Web Ready` / `Web Live` badge whenever SubFrame Server is enabled. That keeps remote-session status visible in the main UI without forcing a trip back into Settings just to see whether the server is running or a client is connected.

That badge then became an action surface rather than a status-only hint. It now exposes `Web Off`, `Web Ready`, `Web Live`, and `Web Error` states with a compact menu for starting/stopping the server and copying connection URLs, while deeper security/network configuration stays in Settings where there is room for warnings and context.

The web server lifecycle was also split into two concepts: current-session runtime state and persistent launch preference. Manual starts from Settings or the activity bar are now session-scoped by default, so using the web UI temporarily on mobile does not automatically cause the server to come back on every future app launch. A separate `Start Server on Launch` setting preserves the old persistent behavior when that is actually wanted.

The follow-up audit on that web-session work exposed a broader same-pattern issue: fixing `event.reply(...)` for browser mode was not enough, because a large part of the older send/on surface still answered with `event.sender.send(...)`. The IPC router now fabricates both sides for browser-originated send handlers so request/response flows like tasks, workspace loads, file-tree loads, and AI-files status checks work over WebSocket without rewriting every handler into invoke mode.
