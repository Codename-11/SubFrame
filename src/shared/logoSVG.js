 ...
 *
 * Usage:
 *   const { getLogoSVG } = require('./logoSVG');
 *   element.innerHTML = getLogoSVG({ size: 140, id: 'splash' });
 */
 ...
 * @param {number}  options.size       - Rendered width/height in px (default: 180)
 * @param {string}  options.id         - Unique ID prefix to avoid SVG filter collisions (default: 'logo')
 * @param {boolean} options.frame      - Include the animated frame outline (default: true)
 * @param {boolean} options.animate    - Include animations (default: true)
 * @param {boolean} options.staticSnap - If true, no animations at all (for static icons)
 */
function getLogoSVG(options = {}) {
  ...
  // Electron motion on each orbit
  const electronMotion = (pathId, dur, begin) => anim
...
/**
 * Logo color palette — exported for use in loading bars, accents, etc.
 */
const LOGO_COLORS = {
  purple: '#b480ff',
  pink: '#ff6eb4',
...
module.exports = { getLogoSVG, LOGO_COLORS };
